...
액션: 어플리케이션에서 어떤 일이 일어나야 하는지를 나타내는 객체로써
주로 사용자 상호작용, API호출, 타이머 등으로 인한 이벤트 등이 있습니다
예) 사용자가 로그인 버튼을 글릭하는 액션이 있다
또는 서버에서 데이터를 성공적으로 받았다 이런 이벤트 등의 액션으로 표현될 수 있습니다
액션은 주로 타입(type)과 그와 필요한 데이터를 포함시킨 객체입니다

리듀서: 리듀서는 현재 상태와 액션을 받아서 새로운 상태를 반환합니다
중요)) 무조건 순수함수입니다
예) 로그인 액션을 처리해서 로그인 상태를 변경해야 합니다
...
스토어에 데이터를 저장하면 동일한 조건으로 같은 어플리케이션 안에서 데이터 접근이 용이합니다
